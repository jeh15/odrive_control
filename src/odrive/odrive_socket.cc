...
namespace base::odrive
{
    ...
    ODriveSocket::ODriveSocket(const base::can::CANSocket::Ptr socket, const Logger::Ptr logger)
    ...
    bool ODriveSocket::setLimits(const uint8_t can_id, const double velocity_limit, const double current_limit)
    {
    ...
    bool ODriveSocket::setPosGain(const uint8_t can_id, const double pos_gain)
    {
    ...
    bool ODriveSocket::setVelGains(const uint8_t can_id, const double vel_gain, const double vel_integrator_gain)
    {
    ...
    bool ODriveSocket::setPosition(const uint8_t can_id, const double pos, const double vel_ff, const double torque_ff)
    {
    ...
    bool ODriveSocket::setVelocity(const uint8_t can_id, const double vel, const double torque_ff)
    {
    ...
    bool ODriveSocket::setTorque(const uint8_t can_id, const double torque)
    {
    ...
    double ODriveSocket::getIqSetpoint(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].iq_setpoint;
    }

    double ODriveSocket::getIqMeasured(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].iq_measured;
    }

    double ODriveSocket::getFETTemperature(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].fet_temperature;
    }

    double ODriveSocket::getMotorTemperature(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].motor_temperature;
    }

    double ODriveSocket::getBusVoltage(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].bus_voltage;
    }

    double ODriveSocket::getBusCurrent(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].bus_current;
    }

    double ODriveSocket::getPosEstimate(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].pos_estimate;
    }

    double ODriveSocket::getVelEstimate(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return info_[can_id].vel_estimate;
    }

    double ODriveSocket::getTorqueEstimate(const uint8_t can_id)
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void ODriveSocket::run()
    {
        ...
        while (isRunning())
        {
            ...
            {
                // Get_Iq
                double iq_setpoint, iq_measured;
            ...
            {
                // Get_Temperature
                double fet_temperature, motor_temperature;
            ...
            case 0x017:
            {
                ...
                double bus_voltage, bus_current;
            ...
            case 0x009:
            {
                ...
                double pos_estimate, vel_estimate;
